...
export default function SettingPage() {
  ...
  const handleToggleCourier = async (data) => {
    ...
    const res = await putData(`couriers/${data.id}`, { name: data.name, status: data.status, website: data.website })
    if (res.data.success) {
      dispatch(fetchCourier())
  ...
  const handleToggleSetting = async (name, value) => {
    ...
    if (name === 'shouldPinPrintTransaction') palyload = { shouldPinPrintTransaction: value }
    ...
    <div className="grid grid-cols-4">
      <div className="col-span-3 flex-col">
          ...
          <div className="col-span-1">
            ...
            <div className="relative mt-4">
              <div className="w-72 h-72 border rounded-lg flex justify-center p-4">
                <img src={`${config.api_host}/images/${setting.data?.photo}`} alt="Photo" />
              </div>
            </div>
          </div>
          <div className="col-span-1">
            <button onClick={() => handleUploadImage()} className="items-center text-white py-4 w-full flex justify-center bg-blue-300 rounded-lg">
              <Image fill={"#FFFFFF"} />
            ...
            <button
              ...
              className="items-center border py-4 w-full flex justify-center bg-white text-neutral-300 rounded-lg mt-4">
              {loading.resetImage ? <div className={`animate-spin`}>
                <CgSpinnerAlt />
              </div> : <Trash fill={"#858585"} />}
        ...
        <div className="mt-20">
                ...
                <li className="flex items-center mb-1" key={i}>
                  <div className="w-full rounded-lg px-6 py-3 flex items-center border justify-between">
                    ...
                    <label className={`wrap-toggle-primary right-7 -top-1`}>
                      <input type="checkbox" defaultChecked={courier.status} onClick={() => handleToggleCourier(courier)} />
                      <span className="toggle-primary"></span>
                      <span className="toggle-default-primary"></span>
                    </label>
                  </div>
                  <Trash fill={"#858585"} className={"mx-5 cursor-pointer"} onClick={() => handleShowRemoveCourier(courier)} />
                  <Edit
          ...
          {isShowCreateCourier && <CreateCourier close={() => setIsShowCreateCourier(false)} />}
          ...
          <ul className="mt-4 w-full">
            <li className="w-full rounded-lg px-6 py-3 flex items-center border justify-between">
              ...
              <label className={`wrap-toggle-primary right-7 -top-1`}>
                <input
                  type="checkbox" defaultChecked={setting.data?.shouldPinAdd}
                  onClick={() => handleToggleSetting('shouldPinAdd', setting.data.shouldPinAdd)}
            ...
            <li className="w-full rounded-lg px-6 py-3 flex items-center border justify-between mt-1">
              ...
              <label className={`wrap-toggle-primary right-7 -top-1`}>
                <input
                  type="checkbox" defaultChecked={setting.data?.shouldPinEdit}
                  onClick={() => handleToggleSetting('shouldPinEdit', setting.data.shouldPinEdit)}
            ...
            <li className="w-full rounded-lg px-6 py-3 flex items-center border justify-between mt-1">
              ...
              <label className={`wrap-toggle-primary right-7 -top-1`}>
                <input
                  type="checkbox" defaultChecked={setting.data?.shouldPinDelete}
                  onClick={() => handleToggleSetting('shouldPinDelete', setting.data.shouldPinDelete)}
                  value={setting.data.shouldPinDelete}
                />
                <span className="toggle-primary"></span>
                <span className="toggle-default-primary"></span>
            ...
            <li className="w-full rounded-lg px-6 py-3 flex items-center border justify-between mt-1">
              ...
              <label className={`wrap-toggle-primary right-7 -top-1`}>
                <input
                  type="checkbox" defaultChecked={setting.data?.shouldPinPrintTransaction}
                  onClick={() => handleToggleSetting('shouldPinPrintTransaction', setting.data.shouldPinPrintTransaction)}
            ...
            <div className="rounded-lg w-460px  bg-white absolute top-1/2 transform left-1/2 -translate-x-1/2 -translate-y-1/2">
              ...
              <div className="p-6">
                <p className="text-center text-2 text-neutral-500">
                  Apakah Anda yakin ingin menghapus data kurir?
                  data yang terlah di hapus tidak dapat dikembalikan.
                </p>
                <button
                  onClick={() => setIsShowAlert(false)}
                ...
                <button onClick={() => handleRemoveCourier(idCourier)} className=" flex justify-center mt-2 text-center bg-white text-blue-300 border border-blue-300 rounded-lg  w-full py-4 focus:outline-none">
                  {loading.removeCourier ? <div className={`animate-spin`}>
                    <CgSpinnerAlt />
                  </div> : ''}
                  Hapus
                </button>
